...
{                                     //выясняющую сколько элементов нужно зарезервировать под массив В,
    ...
    int sizeB = 0;                       //размер массива В - искомая величина, до начала работы обнуляю значение
    ...
    while (index < length)
    {
        if (collection[index] > currentElement)
        {
            ...
            sizeB++;                  // при каждом нахождении правильного элемента, счетчик числа элементов массива В увеличиваю на 1
        }
        index++;
    }
...
int UserInteger() //Метод ожидающий ввода целочисленного числа (здесь - элементов массива) и осуществляющий проверку ввода
{
    int integerNumber = 0; //переменная integerNumber, в данном случае, хранит размер массива А, который задает пользователь
    while (true)
    {
...
string UserString() //Метод ожидающий ввода текстовой строки от пользователя и осуществляющий проверку ввода
{
    Console.Clear(); 
    Console.WriteLine("Введите Ваше имя");
    string stringLine; //=string.Empty; //переменная stringLine  хранит текстовую строку, который задает пользователь
                                   //здесь мы ее обнуляем
    while (true)
    {
        stringLine = Console.ReadLine();
        if (string.IsNullOrWhiteSpace(stringLine)) //Проверка пустого ввода, пробелы или просто ENTER
        {
            Console.WriteLine("Вы ничего не ввели, попробуйте еще раз."); //если текст не введен, запрашиваем по новой
        }
        else
        {
            if (double.TryParse(stringLine, out double number)) //Проверка ввода - число или нет
            {
                Console.WriteLine("Введены числовые значения, попробуйте еще раз.");
            }
            else
            {
                Console.Clear(); 
    ...
    return stringLine;        // возвращаем значение метода (функции) в программу
    }
...
Console.WriteLine($"Привет, {UserString()}");
...
int[] A = new int[Convert.ToInt32(UserInteger())]; //создаю массив А, на основе заданного пользователем числа элементов
                                                   // заполняю его нулями
FillArray(A);
